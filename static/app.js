...
function mostrarApp(user) {
  ...
  const initial = (user.nombre || user.usuario || "U")[0].toUpperCase();
...
// ════════════════════════════════════════════════════════════
// RENDER ALL (carga datos frescos del servidor)
// ════════════════════════════════════════════════════════════
...
async function renderDashboard() {
  ...
  // Construir mapa de fecha → total (rellenar días sin ventas con 0)
  const diasMap = {};
  ...
  chartBar = new Chart(document.getElementById("chart-bar"), {
    type: "bar",
    data: { labels, datasets: [{ label:"Ingresos", data:datos, backgroundColor:"rgba(245,200,0,.7)", borderRadius:6, borderSkipped:false }] },
  ...
  chartPie = new Chart(document.getElementById("chart-pie"), {
    ...
    data: {
      ...
      datasets: [{ data: d.top_productos.map(x=>x.unidades), backgroundColor:["#f5c800","#3b82f6","#22c55e","#e84040","#a855f7"], borderWidth:0 }]
    },
...
async function renderInventario() {
  ...
  document.getElementById("inv-body").innerHTML = products.map(p => {
    ...
    return `<tr>
      <td><strong>${p.nombre}</strong><br><span style="font-size:11px;color:var(--muted)">${p.categoria}</span></td>
      ...
      <td style="display:flex;gap:6px;">
        ...
        <button class="btn btn-danger btn-sm" onclick="openDeleteProduct(${p.id})">Eliminar</button>
      </td>
    </tr>`;
  }).join("");
}
...
async function renderVentas() {
  ...
  document.getElementById("ventas-body").innerHTML = ventas.map((v,i) =>
    `<tr>
      <td style="color:var(--muted)">#${ventas.length-i}</td>
      <td>${v.producto_nombre}</td><td>${v.cantidad}</td>
      <td>${fmt(v.precio_unit)}</td><td>${fmt(v.total)}</td>
      <td><span class="${v.ganancia>=0?"badge badge-green":"badge badge-red"}">${fmt(v.ganancia)}</span></td>
      <td>${new Date(v.fecha).toLocaleString("es-MX",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})}</td>
    </tr>`
  ).join("");
}

// ─── ANÁLISIS ────────────────────────────────────────────────
let chartGanancia=null, chartIngresoPie=null, chartHora=null, chartDiaSem=null;
...
async function renderHistorial() {
  ...
  document.getElementById("hist-total").textContent =
    `${data.length} registros · Total: ${fmt(total)} · Ganancia: ${fmt(gan)}`;
}
...
function openModal(name) {
  // Limpiar form al abrir nuevo producto
...
async function saveProduct() {
  ...
  if (costo < 0 || precio < 0) {
    toast("El costo y el precio no pueden ser negativos.", "error"); return;
  }
  if (precio < costo) {
    toast("⚠️ El precio de venta es menor al costo.", "error"); return;
  }
  try {
    if (id) {
...
async function saveVenta() {
  ...
  // AGREGAR: validar stock antes de llamar al API
  const prod = products.find(x => x.id === pid);
  if (prod && cantidad > prod.stock) {
    toast(`Stock insuficiente. Disponible: ${prod.stock} uds.`, "error"); return;
  }
  try {
...
// ── INICIO ───────────────────────────────────────────────────
checkSession();   // intenta restaurar sesión al cargar la página
